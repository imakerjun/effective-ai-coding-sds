...
> ⚠️ **먼저 정확히 짚고 갑시다.** AI Pro 공식 문서는 Skills를 *"Claude의 skills와 동일한 개념"* 이라고 설명하는데, 여기서 말하는 "Claude의 skills"는 Anthropic의 **Agent Skills**(작업 지침 파일)입니다. Claude Code의 **Sub-agents**(별도 컨텍스트로 격리되어 도는 자식 에이전트)와는 다른 기능입니다.
>
> 즉 AI Pro Skills는 **"이 작업은 이렇게 해"라는 지침을 자동/수동으로 주입**하는 메커니즘이지, 부모 컨텍스트가 자식의 *요약본만 받는* context firewall을 그대로 제공하지는 않습니다. 이 페이지의 핵심 패턴(2.5)을 100% 동일하게 재현하려면 Claude Code의 sub-agent 기능이 필요합니다.
>
> **그럼 AI Pro에서 멀티 에이전트 정신을 어떻게 살리나요?** Skills로 *역할별 지침을 분리*하고 단계마다 명시적으로 호출하는 방식이 현실적인 차선입니다. 완벽한 격리는 아니지만, "한 컨텍스트에 모든 걸 욱여넣지 않는다"는 원칙은 지킬 수 있습니다.
...
### 멀티 에이전트 패턴을 Skills로 *근사하게* 운영하기

Skills는 컨텍스트를 격리하지는 못하지만, **단계별 지침을 분리해서 한 번에 하나만 활성화**하는 방식으로 비슷한 효과를 낼 수 있습니다.

| 강의의 패턴 | Skills로 가능한 부분 | Skills로 안 되는 부분 |
|---|---|---|
| **Plan / Implement / Review 분리** | `plan-only`, `implement-only`, `review-only` 3개 Skill 작성, 단계별 직접 호출 | 같은 대화창에 머무르므로 이전 단계의 토큰이 그대로 남음 |
| **Main + 서브에이전트 (탐색·검증·요약 위임)** | `code-search`, `log-summary` 등을 Skill로 만들어 자동 트리거 | 자식 컨텍스트가 별도로 돌지 않음 — 탐색 결과가 그대로 메인에 누적됨 |
| **에센스 + 변형** | 공통 가이드는 Project Rules, 변형 작업은 각 Skill | — |

> **실전 팁**: 토큰 누적이 부담되는 작업(대규모 코드베이스 탐색, 긴 로그 분석)은 Skill로 묶더라도 **별도 대화창에서 시작**하고, 결과 요약만 메인 작업창에 붙여 넣는 식으로 손으로 firewall을 만들 수 있습니다.
