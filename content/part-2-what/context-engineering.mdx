...
- **Anthropic — [Effective context engineering for AI agents](https://www.anthropic.com/engineering/effective-context-engineering-for-ai-agents)** — "Context engineering = attention budget에 들어갈 high-signal 토큰을 찾는 일"이라는 1차 정의. Pre-load vs Just-in-time 구분의 출처.
- **Anthropic — [Claude Code best practices](https://www.anthropic.com/engineering/claude-code-best-practices)** — `CLAUDE.md`를 프로젝트 컨텍스트의 영속 레이어로 쓰는 실무 가이드.
- **Birgitta Böckeler — [Harness engineering for coding agent users](https://martinfowler.com/articles/exploring-gen-ai/harness-engineering.html)** (Thoughtworks) — 하네스 3-component 중 **Context Engineering**을 첫 번째 축으로 지목.
- **HumanLayer — [Skill Issue: Harness Engineering for Coding Agents](https://www.humanlayer.dev/blog/skill-issue-harness-engineering-for-coding-agents)** — Skills·MCP를 컨텍스트 공급 통로로 설계하는 패턴.
...
> 🛠️ **손으로 익히기**: 이 챕터의 실습은 [실습 트랙 · Step 1 — CLAUDE.md 만들기](./hands-on-1-context)로 분리했습니다. 5분 안에 `/init` → 5섹션 정리 → Before/After 비교까지 진행할 수 있습니다.
