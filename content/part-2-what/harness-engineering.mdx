...
- **Anthropic** — [Effective harnesses for long-running agents](https://www.anthropic.com/engineering/effective-harnesses-for-long-running-agents) Anthropic 엔지니어링 블로그가 직접 "harness" 용어를 사용한 글을 게재했습니다. Claude Code의 설계 철학과 직결됩니다.
...
### 이 강의의 5가지 축 — 출처와 정직한 표기

여기서 강의 신뢰도와 직결되는 부분을 분명히 하고 가겠습니다.

**1차 출처들이 사용하는 분류는 서로 다릅니다.** 어떤 출처도 "5가지 축"이라는 공식 분류를 가지고 있지 않습니다.

| 출처 | 분류 | 구성 요소 |
|---|---|---|
| **Birgitta Böckeler / Martin Fowler 사이트** ([원문](https://martinfowler.com/articles/exploring-gen-ai/harness-engineering.html)) | **3 components** | Context Engineering · Architectural Constraints · Garbage Collection (Entropy Management) |
...
| **Anthropic — Effective harnesses for long-running agents** ([원문](https://www.anthropic.com/engineering/effective-harnesses-for-long-running-agents)) | **3-agent architecture + initializer** | Planner · Generator · Evaluator (+ Initializer 패턴) |
| **HumanLayer** ([원문](https://www.humanlayer.dev/blog/skill-issue-harness-engineering-for-coding-agents)) | **카탈로그** | Skills · MCP · Sub-agents · Hooks · Back-pressure |

**그렇다면 강의의 5가지 축은 어디서 왔는가?**

이 강의의 5가지 축 — Context · Plan · Token · Quality · Multi-Agent — 는 **삼성SDS 의뢰서가 명시한 5개 토픽을 강의가 "하네스 엔지니어링"이라는 한 우산 아래 묶은 것**입니다. SDS 의뢰서 원문(2026년 4월):
...
강의는 이 토픽들을 모두 다루되, **위 1차 출처들의 공통 정신** — *"모델이 아닌 그것을 감싼 환경이 결정한다"* — 으로 묶었습니다. 즉:

- **5가지 토픽 자체**는 SDS 의뢰서가 결정 ✅
- **"하네스 엔지니어링"이라는 우산**은 Böckeler · OpenAI · Anthropic의 공통 흐름에서 빌림 ✅
- **5가지를 한 묶음으로 부르는 분류**는 강의의 프레이밍 ⚠️

> ⚠️ **혼동 주의**: "Harness Engineering의 5가지 축"이라고 말씀드리지만, 이는 *학계나 업계에 정립된 표준 분류*가 아닙니다. **이 강의가 SDS 청자에게 가장 설명하기 좋은 형태로 구성한 분류**입니다. 같은 주제를 다른 분류로도 충분히 잘 가르칠 수 있습니다 — 위 4개 출처가 그 증거입니다.

이렇게 정직하게 표기하는 이유: **틀린 권위에 기대는 것보다, 맞는 출처와 강의 프레이밍을 분리해서 보여주는 것이 더 강합니다.** 청자(특히 시니어 엔지니어)는 권위 인용을 오히려 의심합니다.

### Anthropic Claude Code의 설계 방향

Claude Code가 제공하는 기본 블록 — CLAUDE.md, Plan Mode, 서브에이전트, 훅(Hook) — 은 결국 **개발자가 자신의 하네스를 구축할 수 있도록 돕는** 도구들입니다. SDS 의뢰서가 명시한 5개 토픽이 Claude Code의 기본 기능과 1:1로 매핑되는 이유이기도 합니다.

> **핵심 전환**: "AI에게 무엇을 시킬까"에서 "AI에게 어떤 환경을 줄까"로.

## 강의가 다루는 5가지 토픽의 관계

| 토픽 | 질문 | 핵심 개념 | 1차 출처 매핑 |
|---|---|---|---|
| **2.1 Context Engineering** | 에이전트는 우리 세계를 아는가? | 컨텍스트 계층, CLAUDE.md / AGENTS.md | Böckeler·OpenAI 모두 1번 요소 |
| **2.2 Plan-based Execution** | 실행 전에 합의했는가? | Plan Mode, 역할 분담 | OpenAI "Structured Execution" / Anthropic "Planner" |
| **2.3 Token Optimization** | 길게 일할 수 있는가? | 압축, 분할, 서브에이전트 | Anthropic "context compaction" |
| **2.4 Quality Verification** | "완료"가 검증 가능한가? | TDD, 훅, 리뷰 에이전트 | Böckeler "Architectural Constraints" + Anthropic "Evaluator" |
| **2.5 Multi-Agent Orchestration** | 역할이 분리돼 있는가? | 서브에이전트, Skills, Plan/Impl/Review 분리 | Anthropic 3-agent 아키텍처 / OpenAI "Agent Specialization" |

각 토픽이 어느 출처와 가장 가까운지 매핑해두었습니다. **모든 토픽이 1차 출처에 뿌리가 있고**, 다만 그 5개를 한 묶음으로 묶는 분류는 강의의 프레이밍입니다.

이 5가지는 **순서가 아니라 축**입니다. 동시에 존재해야 하네스가 제대로 작동합니다. 하나라도 빠지면 금방 무너집니다.

- 컨텍스트 없는 플랜 = 엉뚱한 계획
- 플랜 없는 실행 = 안티패턴 1번 (Part 1.4 사례)
